...
SU3::usage="SU3[]: get triplet bases"
SU3B::usage="SU3B[]: get antitriplet bases"
Core::usage="Core[]: get triplet root diagram"
CoreB::usage="CoreB[]: get antitriplet root diagram"

ProductTriplet::usage="ProductTriplet[rootdiagram]: return rootdiagram*triplet"
ProductTripletB::usage="ProductTripletB[rootdiagram]: return rootdiagram*antitriplet"
RootWeightPlot::usage="RootWeightPlot[rootdiagram]: draw root and weight diagram"

SU3WFBuilder::usage="SU3WFBuilder[configuration,symmetry]: all input, all output"
...
(* bases generator *)
IndexRoot[rootdiagram_,T3_,Y_]:=Module[{roots=rootdiagram,x=T3,y=Y},
...
ProductTriplet[rootdiagram_]:=Module[{roots=rootdiagram},
	...
	For[su=1,su<=Length[SU3],su++,For[i=1,i<=Length[roots],i++,
		T3=roots[[i,1]]+Core[[su,1]];
		Y=roots[[i,2]]+Core[[su,2]];
		index=IndexRoot[diagram,T3,Y];
		Basis=ProductBasis[roots[[i,3]],Core[[su,3,1]]];
		If[index==0,
			diagram=Join[diagram,{{T3,Y,Basis}}],
